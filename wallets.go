...
// 创建钱包组
func (wts *Wallets) GenerateWallets() string {
	wt := NewWallet()
...
func GetPubKeyFromAddr(address string) []byte {
	addresByte := base582.Decode(address)
	// 去处校验码、去除version
	length := len(addresByte)
	if length > 0 {
		pubKeyHash := addresByte[1 : length-4]
		return pubKeyHash
	}
	return []byte{}
}
